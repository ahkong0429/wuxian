...
@ccclass
export default class HeroCreateClass extends cc.Component {
  @property(cc.Node)
    ...
    if (this.list.length > 0) {
      for (const key in this.list) {
        ...
        clickEventHandler.target = cc
          .find("Canvas")
          .getChildByName("hero_create_page");
  ...
  buy() {
    const name = cc
      .find("Canvas/hero_create_page/background/name_input")
    ...
    const tips = `创建 [ ${this.activeItem.name} ] 英雄\r\n总共需要 1000 积分`;
    cc.find("Modal")
      .getComponent("Modal")
      .alert({
        content: tips,
        confirmFn: async () => {
          ...
          try {
            const res = await cc
              ...
              .request({
                name: "herocreate",
                data: { race: this.activeItem.code, name: name },
              });
            // await this.reloadItemList();
            cc
              .find("Canvas/hero_create_page/background/name_input")
            ...
            cc.find("Loading").getComponent("Loading").show("herocreating");
          } catch (error) {
            cc.find("Tip")
              .getComponent("Tip")
