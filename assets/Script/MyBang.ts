...
export default class MyBangClass extends cc.Component {
  ...
  async reloadItemList(noloading?: boolean) {
    try {
      const resMap = await cc.find("Network").getComponent("Network").request({
        name: "userduan",
        noloading: noloading,
      });
      this.mapData = resMap;
  ...
  pushNode(item: any, key: any) {
    ...
    //   item.pow +
    //   item.str;
    itemNode.getChildByName("remark").getComponent(cc.Label).string = `No.${
      Number(key) + 1
    }\r\n${this.mapData[item.level]}斗士 - ${item.nickname}\r\n总Rank分：${
      item.rank_sum
    }`;
    // if (item.color == 1) {
