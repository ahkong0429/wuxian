...
@ccclass
export default class MySkillClass extends cc.Component {
  @property(cc.Node)
  ...
  async reloadItemList(noloading?: boolean) {
    try {
      ...
      cc.find("Tip").getComponent("Tip").show("技能仓库获取失败，请重新尝试~");
    }
  }
  ...
  pushNode(item: any, key: any) {
    ...
    //   item.pow +
    //   item.str;
    itemNode.getChildByName("remark").getComponent(cc.Label).string = `${
      item.name
    } LV.${item.level} 强化+${item.lv}\r\n${this.mapData.typeMap[item.type]} ${
      this.mapData.skilltypeMap[item.skilltype]
    } 评分：${item.power}\r\n[ ${
      this.mapData.effecttargetMap[item.effecttarget]
    } ] [ ${this.mapData.skillDistance[item.effectdistance]} ] [ ${
      this.mapData.effecttypeMap[item.effecttype]
    }${this.mapData.effectMap[item.effecttargettype]} ] ${
      this.mapData.statusplusMap[item.statusplus]
    }`;
    // 添加点击事件
